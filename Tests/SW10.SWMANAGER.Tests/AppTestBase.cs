...
using SW10.SWMANAGER.Tests.TestDatas;
using System;
...
namespace SW10.SWMANAGER.Tests
{
    /// <summary>
    /// This is base class for all our test classes.
    /// It prepares ABP system, modules and a fake, in-memory database.
    /// Seeds database with initial data.
    /// Provides methods to easily work with <see cref="SWMANAGERDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<SWMANAGERTestModule>
    {
        ...
        protected void UsingDbContext(Action<SWMANAGERDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SWMANAGERDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SWMANAGERDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SWMANAGERDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SWMANAGERDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SWMANAGERDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SWMANAGERDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SWMANAGERDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SWMANAGERDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SWMANAGERDbContext, Task<T>> func)
        {
            T result;
