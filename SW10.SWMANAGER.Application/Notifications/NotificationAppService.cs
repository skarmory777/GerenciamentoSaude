...
{
    [AbpAuthorize]
    public class NotificationAppService : SWMANAGERAppServiceBase, INotificationAppService
    {
        ...
        {
            using (var _notificationManager = IocManager.Instance.ResolveAsDisposable<INotificationManager>())
            {
                var user = AbpSession.ToUserIdentifier();
                return new UserNotificationsOutputDto(_notificationManager.Object.GetUnreadNotifications(user));
            }
        }
        
        public void SetUserNotificationRead(Guid id)
        {
            if (id == Guid.Empty)
            {
                return;
            ...
            {
                var user = AbpSession.ToUserIdentifier();
                _notificationManager.Object.SetReadMessage(user, id);
            }
        } 
