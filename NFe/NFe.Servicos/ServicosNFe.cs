...
namespace NFe.Servicos
{
    public sealed class ServicosNFe : IDisposable
    {
        ...
        private INfeServicoAutorizacao CriarServicoAutorizacao(ServicoNFe servico)
        {
            ...
            if (_cFgServico.cUF == EstadoNfe.PR & _cFgServico.VersaoNFeAutorizacao == VersaoServico.ve310)
        ...
        private INfeServico CriarServico(ServicoNFe servico)
        {
            ...
            switch (servico)
            {
                case ServicoNFe.NfeStatusServico:
                    if (_cFgServico.cUF == EstadoNfe.PR & _cFgServico.VersaoNfeStatusServico == VersaoServico.ve310)
                    {
                        return new NfeStatusServico3(url, _certificado, _cFgServico.TimeOut);
                    }
                    if (_cFgServico.cUF == EstadoNfe.BA & _cFgServico.VersaoNfeStatusServico == VersaoServico.ve310 &
                ...
                case ServicoNFe.NfeConsultaProtocolo:
                    if (_cFgServico.cUF == EstadoNfe.PR & _cFgServico.VersaoNfeConsultaProtocolo == VersaoServico.ve310)
                    {
                        return new NfeConsulta3(url, _certificado, _cFgServico.TimeOut);
                    }
                    if (_cFgServico.cUF == EstadoNfe.BA & _cFgServico.VersaoNfeConsultaProtocolo == VersaoServico.ve310 &
                        _cFgServico.ModeloDocumento == ModeloDocumento.NFe)
                    {
        ...
        private RetornoRecepcaoEvento RecepcaoEvento(int idlote, List<evento> eventos, ServicoNFe servicoEvento)
        {
            ...
            //            SalvarArquivoXml(string.Format("{0}-ped-eve-{1}-{2}.xml", idlote, DateTime.Now.ToString("yyyyMMdd"), DateTime.Now.ToString("HHmmss")), xmlEvento);
            ...
            //            SalvarArquivoXml(string.Format("{0}-eve-{1}-{2}.xml", idlote, DateTime.Now.ToString("yyyyMMdd"), DateTime.Now.ToString("HHmmss")), retornoXmlString);
            ...
            string justificativa = null)
        {
            ...
            detEvento.descEvento = tipoEventoManifestacaoDestinatario.Equals(TipoEventoManifestacaoDestinatario.TeMdConfirmacaoDaOperacao)
                ? "Confirmacao da Operacao"
                : tipoEventoManifestacaoDestinatario.Equals(TipoEventoManifestacaoDestinatario.TeMdCienciaDaEmissao)
                ? "Ciencia da Operacao"
                : tipoEventoManifestacaoDestinatario.Equals(TipoEventoManifestacaoDestinatario.TeMdDesconhecimentoDaOperacao)
                ? "Desconhecimento da Operacao"
                : tipoEventoManifestacaoDestinatario.Equals(TipoEventoManifestacaoDestinatario.TeMdOperacaoNaoRealizada)
                ? "Operacao nao Realizada"
                : detEvento.descEvento;
            var infEvento = new infEventoEnv
            {
                cOrgao = _cFgServico.cUF == EstadoNfe.RS ? _cFgServico.cUF : EstadoNfe.AN,
        ...
        public RetornoNfeDistDFeInt NfeDistDFeInteresseComChave(string ufAutor, string documento, string chNFe)
        {///TODO: SWMANAGER - CUSTOMIZADO
            var versaoServico = "1.01";
            ...
            var pedDistDFeInt = new distDFeInt2
            {
                versao = versaoServico,
                tpAmb = _cFgServico.tpAmb,
                cUFAutor = _cFgServico.cUF,
                consChNFe = new consChNFe { chNFe = chNFe }
        ...
        public RetornoNfeRecepcao NfeRecepcao(int idLote, List<Classes.NFe> nFes)
        {
            ...
            if (_cFgServico.cUF == EstadoNfe.PR)
        ...
        public RetornoNfeDownload NfeDownloadNf(string cnpj, List<string> chaves)
        {
            ...
            Validador.Valida(ServicoNFe.NFeDistribuicaoDFe, _cFgServico.VersaoNfeDownloadNF, xmlDownload);
