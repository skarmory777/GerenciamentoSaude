...
namespace SW10.SWMANAGER.Web.Controllers
{
    ...
    public class ProfileController : SWMANAGERControllerBase
    {
        ...
        {
            var user = await _userManager.GetUserByIdAsync(AbpSession.GetUserId()).ConfigureAwait(false);
            if (user.ProfilePictureId == null)
            {
                return GetDefaultProfilePicture();
            }

            return await GetProfilePictureById(user.ProfilePictureId.Value).ConfigureAwait(false);
        }
        ...
        public async Task<FileResult> GetProfilePictureById(string id = "")
        {
            ...
            return await GetProfilePictureById(Guid.Parse(id)).ConfigureAwait(false);
        }
