...
using SW10.SWMANAGER.Authorization.Impersonation;
...
using SW10.SWMANAGER.Web.Controllers.Results;
using SW10.SWMANAGER.Web.Models.Account;
...
{
    public class AccountController : SWMANAGERControllerBase
    {
        ...
        {
            var loginResult = await GetLoginResultAsync(loginModel.UsernameOrEmailAddress, loginModel.Password, loginModel.TenancyName).ConfigureAwait(false);
            if (loginResult == null || loginResult.Result == AbpLoginResultType.InvalidPassword
                || loginResult.Result == AbpLoginResultType.InvalidTenancyName
                || loginResult.Result == AbpLoginResultType.InvalidUserNameOrEmailAddress)
            {
                return Json(new AjaxResponse
                {
                    Error = new ErrorInfo("Senha Inválida", "Favor tentar novamente"),
                    Success = false
                });
            }
            ...
            Session["Ambiente"] = loginModel.TenancyName;
            ...
            using (UnitOfWorkManager.Current.SetTenantId(tenantId))
            {
                ...
                if (await _userManager.ValidaControleDeIp(HttpContext.Request.UserHostAddress, loginResult.User).ConfigureAwait(false))
                {
                ...
                return Json(new AjaxResponse
                {
                    Error = new ErrorInfo("Usuário sem permissão de acesso pelo ip"),
                    Success = false
                });
