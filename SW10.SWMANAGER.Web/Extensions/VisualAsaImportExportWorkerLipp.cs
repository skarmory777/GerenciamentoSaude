...
{
    public class VisualAsaImportExportWorkerLipp : PeriodicBackgroundWorkerBase, ISingletonDependency
    {
        ...
        //private IVisualAppService _visualAppService;
        ...
        public VisualAsaImportExportWorkerLipp(
            AbpTimer timer,
        ...
        private async Task ExportarAtendimento(string cnSW)
        {
            using (dbAtendimento = new SWMANAGERDbContext(cnSW))//logo após o teste colocar AMERICAN
            {
                ...
                {
                    //if(item.IDPaciente==null || item.IDAtendimento ==null)
                    //{
                    //    continue;
                    //}
                    ...
                    var query = from ate in dbAtendimento.Atendimentos
                                ...
                                select ate.Paciente.SisPessoa;

                    var pacSw = query.FirstOrDefault();
                    item.IDPaciente = pacSw.ImportaId;
                    ...
                    else //alterar
                    {
                        ...
                        log.IdAsa = item.IDAtendimento.Value;
                        log.Operacao = "ALTERAÇÃO";
                    }
        ...
        {
            using (dbRegExameMov = new SWMANAGERDbContext(cnSW))//logo após o teste colocar AMERICAN
            {
                var exames = await dbRegExameMov
                ...
                for (int i = 0; i < exames.Count(); i++)
                {
                    ...
                    else
                    {
                        ...
                        var exameSw = await dbRegExameMov.Resultados
                            .Include(m => m.Atendimento)
                            .Include(m => m.MedicoSolicitante)
                            .Where(w => w.Id == id)
                            .FirstOrDefaultAsync();
                        exames[i].IdAtendimento = exameSw.Atendimento.ImportaId ?? 0;

                        if (exames[i].IdAtendimento == 0)
                        {
                            var atendimento = await _atendimentoAsaRepositorio.Obter(exameSw.AtendimentoId.Value.ToString());
                            //exame.IdAtendimento = atendimento.IDAtendimento.Value;
                            exames[i].IdAtendimento = atendimento.IDAtendimento.Value;
                        }

                        var idAsa = await _reqExameMovAsaRepositorio.Inserir(exames[i]);

                        dbRegExameMov.Database.ExecuteSqlCommand("UPDATE LABRESULTADO SET IMPORTAID=" + idAsa + " WHERE ID=" + exames[i].IDSW.Value);
                        dbRegExameMov.Database.ExecuteSqlCommand("UPDATE PRO_REQEXAMEMOVITEM SET IDREQUISICAOMOV=" + idAsa + " WHERE PRO_REQEXAMEMOVID=" + exames[i].IDSW.Value + " AND ISDELETED=0");

                        log.IdAsa = idAsa;
                        log.Operacao = "INCLUSÃO";
                    }
                    dbRegExameMov.Entry(exames[i]).State = EntityState.Deleted;
                    dbRegExameMov.VisualAsaImportExportLogs.Add(log);
                    dbRegExameMov.SaveChanges();
                }
                //CurrentUnitOfWork.SaveChanges();
                dbRegExameMov.Dispose();
            }
        }
        ...
        private async Task ExportarReqExameMovItem(string cnSW)
        {
            ...
            {
                var exameItens = await dbRegExameMovItem
                    .Pro_ReqExamesMovItensVisualASA
                    //.Where(m => m.TenantId == tenantId)
                    .ToListAsync();

                for (int i = 0; i < exameItens.Count(); i++)
                {
                    var log = new VisualAsaImportExportLog();
                    log.Tabela = "PRO_REQEXAMEMOVITEM";
                    log.IdSw = exameItens[i].IDSW.Value;

                    var exameItem = await _reqExameMovItemAsaRepositorio.Obter(exameItens[i].IDSW.Value.ToString());
                    if (exameItem != null && (exameItem.IdRequisicaoMovItem > 0 && exameItem.IDSW.HasValue))
                    {
                        ...
                        //}
                        exameItens[i].IdRequisicaoMov = exameItem.IdRequisicaoMov;
                        exameItens[i].IdRequisicaoMovItem = exameItem.IdRequisicaoMovItem;
                        exameItens[i].IdItem = exameItem.IdItem;
                        await _reqExameMovItemAsaRepositorio.Alterar(exameItens[i]);
                        log.IdAsa = exameItens[i].IdRequisicaoMovItem;
                        log.Operacao = "ALTERAÇÃO";
                    }
                    else
                    {

                        var id = exameItens[i].IDSW.Value;
                        ...
                        //contasmedicas[i].IDContaMedica = ateSw.ImportaId;
                        exameItens[i].IdItem = exameSw.FaturamentoItem.ImportaId ?? 0;
                        //exames[i].IdRequisicaoMov = exameSw.Resultado.ImportaId ?? 0;

                        var resExame = await _reqExameMovAsaRepositorio.Obter(exameSw.ResultadoId.ToString());
                        exameItens[i].IdRequisicaoMov = resExame.IdRequisicaoMov;

                        var idAsa = await _reqExameMovItemAsaRepositorio.Inserir(exameItens[i]);
                        dbRegExameMovItem.Database.ExecuteSqlCommand("UPDATE LABRESULTADOEXAME SET IMPORTAID=" + idAsa + " WHERE ID=" + exameItens[i].IDSW.Value);
                        log.IdAsa = idAsa;
                        log.Operacao = "INCLUSÃO";
                    }
                    dbRegExameMovItem.Entry(exameItens[i]).State = EntityState.Deleted;
