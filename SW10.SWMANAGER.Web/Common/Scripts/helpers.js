...
(function () {
    ...
    var appLocalizationSource = abp.localization.getSource('SWMANAGER');
    ...
    app.capitalize = function capitalize(str) {
        var excludeList = ["o", "os", "a", "as", "e", "do", "dos", "de", "da", "das", "se"];
        var words = str.split(' ');
        var capitalized = [];

        words.forEach(function (each) {
            console.log(each.toLowerCase());
            if (excludeList.indexOf(each.toLowerCase()) !== -1) {
                capitalized.push(each.toLowerCase());
            } else {
