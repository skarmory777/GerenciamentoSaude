...
{
    public class ProdutosUnidadeController : Controller // SWMANAGERControllerBase
    {
        private readonly IUnidadeAppService _unidadeAppService;

        public ProdutosUnidadeController(
        ...
        [HttpPost]
        public JsonResult ListarUnidadesOptions()
        {
            try
            {
        ...
        {
            var query = await _unidadeAppService.ListarAutoComplete(term);
            return Json(query.Items.ToArray(), JsonRequestBehavior.AllowGet);
        }


        public async Task<JsonResult> AutoCompleteNaoUtilizado(string term, long produtoId)
        {
            var query = await _unidadeAppService.ListarAutoComplete(term);
            return Json(query.Items.ToArray(), JsonRequestBehavior.AllowGet);
        }
