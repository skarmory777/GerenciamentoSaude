(function ($) {
    app.modals.CriarOuEditarBancoAgenciasModal = function () {
        ...
        var _modalManager;
        var _$bancoAgenciasInformationsForm = null;
        ...
        this.init = function (modalManager) {
            ...
            if (_args.id != undefined) {
                _bancoAgenciasService.obter(_args.id)
                 .done(function (data) {
                     $('#bancoId').val(data.id);
                     $('#codigo').val(data.codigo);
                     $('#descricao').val(data.descricao);
                     for (var i = 0; i < data.agencias.length; i++) {
                         lista.push({
                                IdGrid: lista.length == 0 ? 1 : lista[lista.length - 1].IdGrid + 1,
                                Id: data.agencias[i].id,
                                BancoId: data.agencias[i].bancoId,
                                Codigo: data.agencias[i].codigo,
                                Descricao: data.agencias[i].descricao
                         });                         
                     }
                     getSubGrupoTable();
                 });
            ...
            _$bancoAgenciasInformationsForm = _modalManager.getModal().find('form[name=bancoAgenciasInformationsForm]');
            _$bancoAgenciasInformationsForm.validate();
        };

        this.save = function () {

            if (!_$bancoAgenciasInformationsForm.valid()) {
                return;
            }

            var bancoAgencias = _$bancoAgenciasInformationsForm.serializeFormToObject();

            bancoAgencias.agencias = lista;
            _modalManager.setBusy(true);

            App.startPageLoading({ animate: true }); document.querySelector('.loadingCommon').style.display = null;
            _bancoAgenciasService.criarOuEditar(bancoAgencias)
                 .done(function (data) {
                     App.stopPageLoading(); document.querySelector('.loadingCommon').style.display = 'none';
        ...
        $('#inserir').click(function (e) {
            ...
            var _$LocalChamadaInformationsForm = $('form[name=BancoAgenciasInformationsForm]');
       ...
       ({
           ...
           //actions: {
           //    listAction: {
           //        method: _bancoAgenciasService.listarLocalChamada
           //    }
           //},
