(function () {
    $(function () {
        ...
        document.addEventListener('visibilitychange', function () {
            var activeLi = $("#abas-int").find("li.active");
        ...
        var _registrosArquivos = new app.ModalManager({
            ...
            focusFunction : (_$modal) => { debugger }
        });
        ...
        _$AssistencialAtendimentosTable.jtable({
            ...
            fields: {
                            ...
                            .click(function () {
                                ...
                                if (data.record.senha && data.record.senha !== "") {
                                    swal({
                                        ...
                                        window.open(url + "?id=" + data.record.id);
                                        $('.jtable-row-selected').removeClass('jtable-row-selected');
                                        $(':checked').removeAttr('checked');
                                    });
                                } else {
                ...
                'AteClassificacaoAtendimento.Descricao': {
                ...
                "SisPessoaPaciente.NomeCompleto": {
                ...
                'SisTipoAcomodacao.Descricao': {
                    title: app.localize('Tipo de Leito'),
                    width: '10%',
                    display: function (data) {
                        var nome = '';
                        if (data.record.tipoLeito !== null) {
                            return data.record.tipoLeito;
                        }
                        return nome;
                    }
                },
                'AteProtocoloAtendimento.Descricao': {
        ...
        $(".unidade").click((event) => {
            let checked = $(event.target).data('checked');
            $(event.target).data('checked', !checked);

            if ($(event.target).data('checked')) {
        ...
        function updateGrid() {
            var listUnidade = [];
            $.each($(".unidade"), (index, item) => {
                if ($(item).data('checked')) {
                    listUnidade.push($(item).data('value'));
                }
            });

            const params = createRequestParams();

            params.unidadeOrganizacionais = listUnidade;

            _$AssistencialAtendimentosTable.jtable('load', params);
        }
