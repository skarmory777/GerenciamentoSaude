(function ($) {
    app.modals.CriarOuEditarPreMovimentoItemModal = function () {
        ...
        this.init = function (modalManager) {
            var currencyFields = [
                'custoUnitarioId', 'custoTotalId', 'valorIPIId', 'valorICMSId'
            ];
            var numericFields = ['QuantidadeItemid', 'perIPIId'];
            var percFields = ['perIPIId', 'perICMSId'];
            ...
            _.forEach(percFields, (item) => {
                const el = document.querySelector(`#${item}`);
                if (el) {
                    maskFields[item] = IMask(el, percMaskTemplate);
                }
            })
            ...
            if (localStorage['inventario'] == '19') {
                ...
                maskFields['perICMSId'].unmaskedValue = '0';
                maskFields['valorICMSId'].unmaskedValue = '0';
                ...
                document.querySelector('.impostos').style.display = 'none';
        ...
        this.save = function () {
            ...
            preMovimentoItem.perICMS = maskFields['perICMSId'].unmaskedValue;
            preMovimentoItem.ValorICMS = maskFields['valorICMSId'].unmaskedValue;
            debugger;
        ...
        $('#perICMSId').focusout(function (e) {
            e.preventDefault();
            calcularValorICMS();
        });

        $('#valorICMSId').focusout(function (e) {
            e.preventDefault();
            calcularPerICMS();
        });

        function calcularValorICMS() {
            var perICMS = ($('#perICMSId').val() != '') ? numeral(parseFloat(maskFields['perICMSId'].unmaskedValue)).value() : 0;
            var custoTotal = ($('#custoTotalId').val() != '') ? numeral(parseFloat(maskFields['custoTotalId'].unmaskedValue)).value() : 0;
            var valorICMS = perICMS * (custoTotal / 100);

            if (isNaN(valorICMS)) {
                valorICMS = 0;
            }
            maskFields['valorICMSId'].unmaskedValue = valorICMS.toString();
        }
        function calcularPerICMS() {

            var custoTotal = ($('#custoTotalId').val() != '') ? numeral(parseFloat(maskFields['custoTotalId'].unmaskedValue)).value() : 0;
            var valorICMS = ($('#valorICMSId').val() != '') ? numeral(parseFloat(maskFields['valorICMSId'].unmaskedValue)).value() : 0;
            var perICMS = (valorICMS * 100) / custoTotal;

            if (isNaN(perICMS)) {
                perICMS = 0;
            }
            maskFields['perICMSId'].unmaskedValue = perICMS.toString();
        }
