(async function ($) {
    ...
    var _contasRecebidasModal = new app.ModalManager({
        viewUrl: abp.appPath + 'Mpa/ContasReceber/AbrirModalContasRecebidas',
        scriptUrl: abp.appPath + 'Areas/Mpa/Views/Aplicacao/Financeiros/ContasReceber/ContasRecebidas/_ContasRecebidasModal.js'
    })

    $('#editConvenioButton').click(function (e) {
        e.preventDefault();
        var pessoaId = $("#pessoaId").val();
        if (pessoaId != "") {
            _contasReceberService.obterConvenioId(pessoaId)
                .done(function (data) {
                    if (data != null) {
    ...
    $('#salvar').click(function (e) {
        ...
        _contasReceberService.criarOuEditar(contasPagar)
            .done(function (data) {
                if (data.errors.length > 0) {
                    ...
                    location.href = '/mpa/ContasReceber';
    ...
    _$quitacoesTable.jtable({
        ...
        fields: {
            ...
            Actions: {
                ...
                display: function (data) {
                    var $span = $('<span></span>');
                    if (data.record.possuiEntregaContaRecebida) {
                        $('<button class="btn btn-default btn-xs" type="button" title="' + app.localize('ContasRecebidas') + '"><i class="fa fa-file-alt"></i></button>')
                            .appendTo($span)
                            .click(function () {
                                _contasRecebidasModal.open({ quitacaoId: data.record.id });
                            });
                    }
                    return $span;
                }
    ...
    async function getQuitacoes(reload) {
