    ...
    $(function () {
        ...
        var chat = {
            ...
            getFriendOrNull: function (userId, tenantId) {
                var friend = _.filter(chat.friends, { friendUserId: parseInt(userId), friendTenantId: tenantId ? parseInt(tenantId) : null });
                if (friend.length) {
                    return friend[0];
                }
            ...
            renderFriendLists: function (friends) {
                ...
                var acceptedFriends = _.filter(friends, { state: app.consts.friendshipState.accepted });
                chat.renderFriendList(acceptedFriends, $('#friendListFriends'));

                var blockedFriends = _.filter(friends, { state: app.consts.friendshipState.blocked });
            ...
            registerEvents: function () {
                ...
                abp.event.on('app.chat.friendshipRequestReceived', function (data, isOwnRequest) {
                    ...
                    if (!_.filter(chat.friends, { userId: data.friendUserId, tenantId: data.friendTenantId }).length) {
            ...
            user: {
                ...
                markAllUnreadMessagesOfUserAsRead: function (user) {
                    ...
                    var unreadMessages = _.filter(user.messages, { readState: app.chat.readState.unread });
